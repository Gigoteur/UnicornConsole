...
pub mod plugin {
    ...
    def sound_load(&self, filename: String) -> PyResult<i32> {
        Ok(self.sound(py).lock().unwrap().sound_load(filename))
    }

    def sound_play(&self, filename: String, loops: i32, channel: i32) -> PyResult<i32> {
        self.sound(py).lock().unwrap().sound_play(filename, loops, channel);
        Ok(0)
    }

    def sound_pause(&self, channel: i32) -> PyResult<i32> {
        self.sound(py).lock().unwrap().sound_pause(channel);
        Ok(0)
    }

    def sound_resume(&self, channel: i32) -> PyResult<i32> {
        self.sound(py).lock().unwrap().sound_resume(channel);
        Ok(0)
    }

    def sound_stop(&self, channel: i32) -> PyResult<i32> {
        self.sound(py).lock().unwrap().sound_stop(channel);
        Ok(0)
    }

    def sound_volume(&self, volume: i32, channel: i32) -> PyResult<i32> {
        self.sound(py).lock().unwrap().sound_volume(volume, channel);
        Ok(0)
    }

    def sound_isplaying(&self, channel: i32) -> PyResult<bool> {
        Ok(self.sound(py).lock().unwrap().sound_isplaying(channel))
    }
