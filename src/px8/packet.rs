     ...
     ($($dir:ident $dirName:ident {
        ...
        pub mod $dir {
            ...
            pub mod $state {
                ...
                use std::io::Error;
                //use utils::Error;
                use px8::packet::PacketType;
                use px8::packet::Serializable;
        ...
        pub fn packet_by_id<R: io::Read>(dir: Direction, state: State, id: i32, mut buf: &mut R) -> Result<Option<Packet>, Error> {
            match dir {
                $(
                    Direction::$dirName => {
                        match state {
                            $(
                                State::$stateName => {
                                    match id {
                                    $(
                                        self::$dir::$state::internal_ids::$name => {
                                            use self::$dir::$state::$name;
...
pub fn read_packet(buf: Vec<u8>) -> Result<Packet, Error> {
   ...
   // try!(buf.read_exact(&mut ibuf));
    ...
    let packet = try!(packet_by_id(Direction::Client, State::Gfx, id, &mut buf_cur));
